import {MODULE_METADATA_KEY, ModuleMetadata} from "../decorators/module";
import {container, DESIGN_PARAM_TYPES_METADATA_KEY, INJECT_TOKENS_KEY} from "../di/container";
...
import {PIPES_METADATA_KEY} from "../decorators/use-pipe";
...
export class ModuleScanner {
    ...
    private readonly scannedModules = new Set<any>();

    constructor(private readonly app: Express ) {}

    async scan(moduleClass: any) {

        if (this.scannedModules.has(moduleClass)) {
            return;
        }

        this.scannedModules.add(moduleClass);
    ...
    private registerControllers(controllers: any[]) {
        controllers.forEach((ControllerClass) => {
            ...
            const paramTypes: any[] =
                Reflect.getMetadata(DESIGN_PARAM_TYPES_METADATA_KEY, ControllerClass) || [];
